...
Fetch scoring period id for team
    ...
    ${teams_response}=    GET  url=${TEAM_API}=${team_id1}      headers=${header}      expected_status=200
...
Validate players are added and dropped from ${response}
    ...
    ${player1s_toTeamId}  Get value from JSON       ${response.json()}        $.items[0].toTeamId    
    ${player2s_fromTeamId}    Get value from JSON     ${response.json()}     $.items[1].fromTeamId   
